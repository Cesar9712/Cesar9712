...
function score(l) {
  ...
  const reject = ['minimum deposit','trade volume','trading activity','perps','leverage','seed phrase','private key','token sale','buy token','stake funds'].some(x => t.includes(x));
  if (reject) return { score: -999, classification: 'DESCARTADA' };
  let s = 50;
  ...
  if (t.includes('twitter') || t.includes('tweet')) s -= 10;
  if (t.includes('video') || t.includes('loom')) s -= 5;
  if (t.includes('wallet') || t.includes('sign transaction')) s -= 12;
  return { score: s, classification: s >= 80 ? 'CONFIANZA ALTA' : s >= 65 ? 'CONFIANZA MEDIA' : s >= 45 ? 'CONFIANZA BAJA' : 'DESCARTADA' };
}
...
async function scan() {
  ...
  const paths = [
    ...
    '/api/agents/listings/live?take=50&deadline=2026-12-31'
  ];
  let data = null;
  let last = null;
  for (const p of paths) {
    try { data = await request(p); break; } catch (e) { last = e; console.error(JSON.stringify({ event:'scan_attempt_failed', path:p, message:e.message })); }
  }
  ...
  state.opportunities = raw.map(x => ({ ...x, operatorScore: score(x) })).sort((a,b) => b.operatorScore.score - a.operatorScore.score);
  state.lastScanAt = new Date().toISOString();
  state.lastError = null;
  const top = state.opportunities.slice(0, 12).map(x => ({
  ...
  console.log(JSON.stringify({ event: 'scan_complete', count: state.opportunities.length, at: state.lastScanAt, top }));
...
http.createServer(async (req,res) => {
  try {
    ...
    if (url.pathname === '/admin/submit') {
      let eligibilityAnswers=[]; const raw=url.searchParams.get('eligibilityAnswers'); if(raw) eligibilityAnswers=JSON.parse(raw);
      const out=await submit({slug:url.searchParams.get('slug')||'',listingId:url.searchParams.get('listingId')||'',link:url.searchParams.get('link')||'',tweet:url.searchParams.get('tweet')||'',otherInfo:url.searchParams.get('otherInfo')||'',telegram:url.searchParams.get('telegram')||'',ask:url.searchParams.get('ask'),eligibilityAnswers});
